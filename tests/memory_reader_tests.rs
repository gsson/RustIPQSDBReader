...
use ipqs_db_reader::{MemoryReader, Strictness};
...
fn ipv6() -> Result<(), Box<dyn Error>> {
    ...
    let ip = IpAddr::from_str(IPV6_EXAMPLE)?;
...
#[test]
fn specific_details() -> Result<(), Box<dyn Error>> {
    let mut path_buf = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    path_buf.push("resources/IPQualityScore-IP-Reputation-Database-IPv4.ipqs");
    let reader = MemoryReader::open(&path_buf)?;
    let ip = IpAddr::from_str(IPV4_EXAMPLE)?;
